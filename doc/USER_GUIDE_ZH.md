...
韩式条漫按固定高度切片，两种漏检无法靠调阈值解决：

1. 气泡跨界，只剩半截轮廓 → 被 `ignore_bubble` 当"气泡外文字"过滤
2. **一行字的上下两半分落两页** → 单页内根本没有完整字形，检测和 OCR 都失败

作品配置里的 `ocr_boundary_overlap`（像素）控制这个行为：识别每页时临时拼上
**上页尾部 + 本页 + 下页头部**，行完整后再把坐标裁剪回本页；跨界的行会在相邻
两页各留一份裁剪后的区域，两侧都各自擦掉自己的一半。代价是 OCR 像素量约
+2×overlap/页（540 页实测约 +10% 耗时），`0` 表示关闭。
详见[作品配置 §2.6](SERIES_CONFIG.md#26-分页拼接韩式条漫)。
